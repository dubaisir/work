select CASE WHEN t1.region IS NULL THEN  '集团' ELSE t1.region END  as   region
,CASE WHEN t1.region IS NOT NULL AND  t1.projectname IS NULL THEN '地区合计'
      ...
      ELSE t1.projectname END  as projectname
,t1.bud_account
...
from 
          ...
          (
           SELECT
                  ...
                  ,sum(case when a.BUD_ACCOUNT = '成本' then a.BUD_VAL else  0 end) as  BUD_VAL_CB
...
LEFT JOIN (select  CONCAT(CONCAT(20,substr(a.bud_year,3,2)),lpad(REPLACE(a.bud_period,'月',''),2,0) ) as ym
         ,'货值' AS TYPE
         ,sum(case when a.bud_account  = '货值'   then  a.bud_val else 0   end )  as syz
...
select  CONCAT(CONCAT(20,substr(a.bud_year,3,2)),lpad(REPLACE(a.bud_period,'月',''),2,0) ) as ym
                    ,'成本' AS TYPE
                     ,sum(case when a.bud_account    = '成本'          then  a.bud_val else 0       end )  as syz
...
select  CONCAT(CONCAT(20,substr(a.bud_year,3,2)),lpad(REPLACE(a.bud_period,'月',''),2,0) ) as ym
                     ,'权益后利润' AS TYPE 
                     ,sum(case when a.bud_account  = '权益后利润'  then  a.bud_val else 0 end )        as syz --权益后利润
                   from pln_inter_2_jfh.exp_data_jfh_d  a
                   where
                   bud_period not in ('上半年','下半年','全年') and
                   BUD_ACCOUNT in ('成本','货值','权益后利润') AND
