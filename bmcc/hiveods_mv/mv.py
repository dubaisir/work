...
class Ftp:
    ...
    def get_new_filename(prefix):
        filename_time = datetime.datetime.now().strftime("%Y%m%d%H%M")
        file_new_name = os.path.join(Conf.hdfspath, Conf.localhostname + "_" + prefix + "_" + filename_time+"_" + "%s" % (int(random.random() * 10000)) + "." + Conf.suffix)
    ...
    def mv_file(self):
        ...
        try:
            print(datetime.datetime.now().strftime("%Y%m%d %H:%M:%S") + "   move file to " + mv_path)
            for filename in os.listdir(Conf.localpath):
                if os.path.isdir(os.path.join(Conf.localpath, filename)):
                    pass
                elif re.match(".*chk",filename) or re.match(".*CHK",filename):
                    os.remove(os.path.join(Conf.localpath,filename))
                else:
                    shutil.move(os.path.join(Conf.localpath, filename), mv_path)
        except:
            print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "Mv error")
    ...
    def uploadfile(self, localpath, suffix):
        ftp = Ftp()
        busize = 1024
        while True:
            for prefix_type in Conf.prefix:
                start_time = time.time()
                size = 0
                remotefilename = ftp.get_new_filename(prefix_type)
                for filename in os.listdir(os.path.join(localpath, Conf.localhostname)):
                    if re.match("%s.*%s" % (prefix_type, suffix), filename):  # 匹配文件名
                        ...
                        print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") +"   "+ filename+"   " + "uploading")
...
if __name__ == '__main__':
    ...
    on = Ftp()
    on.connect()
    on.uploadfile(Conf.localpath, Conf.suffix)
